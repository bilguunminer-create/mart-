...
--    total delivered, non-demo spend -- including this order, since its
--    status is already set to Дууссан earlier in this same function -- and
--    use that tier's own cashback_pct. Customers who haven't reached a tier
--    yet earn at the admin-configured base rate (loyalty_cashback_pct,
--    default 1%). If the admin hasn't saved custom tiers yet, falls back to
--    the same default thresholds/rates as LOYALTY_TIERS in
--    src/data/storeData.ts. Also includes a one-time backfill correction for
--    orders already delivered under the old hardcoded 1% formula.
--
