...
--
-- This targets the shared Supabase project behind uskmart.com, the same one
-- the sync-updates branch's copy of this file already fixed -- included here
-- too so this codebase's own migration history reflects it. Safe to run
-- again even if it was already applied: CREATE OR REPLACE is idempotent and
-- the backfill only touches rows where email is still null.
--
