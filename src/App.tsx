...
export default function App() {
  ...
  // Хуучин 'gobi_mart_cart' key-г migration хийж шинэ key руу шилжүүлнэ.
  const [cart, setCart] = useState<CartItem[]>(() => {
    try {
      // Хуучин key-с шилжүүлэлт
  ...
  useEffect(() => {
    ...
    // P-01 fix: 20с → 45с болгосон (20с тутамд 3 RPC → Supabase-д их ачаалал).
    // Canaл захиалгыг шуурхай харахын тулд Supabase Realtime subscription руу
    // шилжүүлэх нь илүү оновчтой боловч одоогоор энэ interval хангалттай.
    const timer = window.setInterval(() => { if (!document.hidden) void loadCentralData(); }, 45000);
      ...
      <CheckoutModal
        ...
        onOrderSuccess={async (order) => {
          ...
          const savedOrder = await saveStoreOrder(currentUser.accessToken, {
            ...
            address: order.address,
            notes: order.notes,
            // F-01 fix: email-г дамжуулж DB-д хадгалах
