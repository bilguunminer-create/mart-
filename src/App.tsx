...
export default function App() {
  ...
  useEffect(() => {
    ...
    let active = true;
    const loadCentralData = () => Promise.allSettled([getStoreOrders(currentUser.accessToken), getStoreCustomerProfiles(currentUser.accessToken)])
      .then(([ordersResult, profilesResult]) => {
        if (!active) return;
        const failures: string[] = [];

        if (ordersResult.status === 'fulfilled') {
          setOrders(ordersResult.value.map((order) => ({
            orderId: order.id,
